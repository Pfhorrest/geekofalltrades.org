...
import { findSubgalleryLink, parseSubgalleryEntries, } from "./getSubgalleryData";
...
describe("parseSubgalleryEntries", () => {
    ...
    `;
        const [entry] = parseSubgalleryEntries(html, "https://geekofalltrades.org/photos/2026/09/03/");
        expect(entry.title).toBe("Curlew in the Sunset");
        expect(entry.description).toBe("iPhone 14 Pro Max, 2026-09-03");
        expect(entry.imgSrc).toBe("https://geekofalltrades.org/photos/2026/09/03/IMG_3332-thumb.jpeg");
        expect(entry.imgAlt).toBe("A curlew wading");
        expect(entry.fullImageSrc).toBe("https://geekofalltrades.org/photos/2026/09/03/IMG_3332.jpeg");
    ...
    `;
        const [entry] = parseSubgalleryEntries(html, "https://geekofalltrades.org/photos/2026");
        expect(entry.imgSrc).toBe("https://geekofalltrades.org/photos/2026/06/20/IMG_1763-thumb.jpeg");
    });
    ...
    `;
        const [entry] = parseSubgalleryEntries(html, "https://geekofalltrades.org/photos/2026/09/04/");
        expect(entry.title).toBe('Untitled <span class="maybe">(Maybe: Bee)</span>');
    });
    ...
    `;
        const entries = parseSubgalleryEntries(html, "https://geekofalltrades.org/photos/2026/09/");
    ...
    `;
        const [entry] = parseSubgalleryEntries(html, "https://geekofalltrades.org/photos/2026/09/");
...
describe("findSubgalleryLink", () => {
    ...
    `);
        ...
        expect(link === null || link === void 0 ? void 0 : link.pathname).toBe("/photos/2026/09/04");
    });
    ...
    `);
        const link = findSubgalleryLink(item);
        expect(link === null || link === void 0 ? void 0 : link.pathname).toBe("/photos/2026/02/");
    });
    ...
    it("returns null when the item has neither link", () => {
        const item = createItemFixture(`<div class="item"><p class="title">No links here</p></div>`);
        expect(findSubgalleryLink(item)).toBeNull();
    });
});
//# sourceMappingURL=getSubgalleryData.test.js.map